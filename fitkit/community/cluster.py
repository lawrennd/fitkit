...
class SpectralCluster(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, y=None):
        ...
        n = self.laplacian_matrix_.shape[0]
        k_eigs = min(self.max_clusters + 5, n - 2)
        
        try:
            L_sparse = sparse.csr_matrix(self.laplacian_matrix_)
            eigvals, eigvecs = eigsh(L_sparse, k=k_eigs, which='LA')
        except Exception:
